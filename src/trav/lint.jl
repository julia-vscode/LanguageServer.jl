...
function lint(x::EXPR{IDENTIFIER}, s::TopLevelScope, L::LintState, server, istop)
    ...
    if x.val == "FloatRange" && !(nsEx in keys(s.symbols))
        push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.fullspan), [], "Use of deprecated `FloatRange`, use `StepRangeLen` instead."))
        
        push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:x.fullspan), "StepRangeLen"))
    end
...
function lint(x::EXPR{CSTParser.Call}, s::TopLevelScope, L::LintState, server, istop)
    if x.args[1] isa EXPR{IDENTIFIER}
        ...
        if x.args[1].val == "write" && length(x.args) == 4 && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function form"))

            # may need fixing for triple quoted strgins
            arg = CSTParser.isstring(x.args[3]) ? string('\"', x.args[3].val, '\"') : Expr(x.args[3])
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:x.fullspan), string("write(STDOUT, ", arg,")")))
        # l129 : 3 arg version of `delete!`
        elseif x.args[1].val == "delete!" && length(x.args) == 8 && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "`delete!(ENV, k, def)` should be replaced with `pop!(ENV, k, def)`. Be aware that `pop!` returns `k` or `def`, while `delete!` returns `ENV` or `def`."))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:x.args[1].fullspan), "pop!"))
        # l372 : ipermutedims
        elseif x.args[1].val == "ipermutedims" && length(x.args) == 6 && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function"))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + sum(x.args[i].fullspan for i = 1:4) + (0:x.args[5].fullspan), string("invperm(", Expr(x.args[5]), ")")))
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:x.args[1].fullspan), "permutedims"))
        # l381 : is(a, b) -> a === b
        elseif x.args[1].val == "is" && length(x.args) == 6 && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function"))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + sum(x.args[i].fullspan for i = 1:5) + (0:x.args[6].fullspan), ""))
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + sum(x.args[i].fullspan for i = 1:3) + (0:x.args[4].fullspan), " === "))
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:sum(x.args[i].fullspan for i = 1:2)) , ""))
        # l465
        elseif x.args[1].val == "den" && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function"))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:x.args[1].fullspan), "denominator"))
        # l466
        elseif x.args[1].val == "num" && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function"))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:x.args[1].fullspan), "numerator"))
        # l471
        elseif x.args[1].val == "takebuf_array" && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function"))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:x.args[1].fullspan), "take!"))
        # l472
        elseif x.args[1].val == "takebuf_string" && length(x.args) == 4 && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function"))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + sum(x.args[i].fullspan for i = 1:3) + (0:x.args[4].fullspan), "))"))
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:x.args[1].fullspan), "String(take!"))
        # l527/528 : sumabs(args...) -> sum(abs, args...)
        elseif x.args[1].val == "sumabs" && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function"))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:sum(x.args[i].fullspan for i = 1:2)) , "sum(abs, "))
        # l529/530 : sumabs2(args...) -> sum(abs2, args...)
        elseif x.args[1].val == "sumabs2" && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function"))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:sum(x.args[i].fullspan for i = 1:2)) , "sum(abs2, "))
        # l531/532 : minabs(args...) -> minimum(abs, args...)
        elseif x.args[1].val == "minabs" && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function"))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:sum(x.args[i].fullspan for i = 1:2)) , "minimum(abs, "))
        # l533/534 : maxabs(args...) -> maximum(abs, args...)
        elseif x.args[1].val == "maxabs" && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function"))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:sum(x.args[i].fullspan for i = 1:2)) , "maximum(abs, "))
        elseif x.args[1].val == "quadgk" && !(nsEx in keys(s.symbols))
            ...
            if !isimported
                push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "`quadgk` has been moved to the package QuadGK.jl.\nRun Pkg.add(\"QuadGK\") to install QuadGK on Julia v0.6 and later, and then run `using QuadGK`."))
            end
        elseif x.args[1].val == "bitbroadcast" && !(nsEx in keys(s.symbols))
            push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.args[1].fullspan), [], "Use of deprecated function"))
            
            push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit(s.current.offset + (0:x.args[1].fullspan), "broadcast"))
        elseif x.args[1].val == "include"
            ...
            if !(isincludable(x) && uri in keys(server.documents))
                ...
                push!(L.diagnostics, CSTParser.Diagnostics.Diagnostic{CSTParser.Diagnostics.PossibleTypo}(s.current.offset + (0:x.fullspan - tws), [], "Could not include $file"))
            end
...
function _lint_sig(sig, s, L, fname, offset)
    if sig isa EXPR{Call} && sig.args[1] isa EXPR{CSTParser.Curly} && !(sig.args[1].args[1] isa EXPR{CSTParser.InvisBrackets} && sig.args[1].args[1].args[2] isa EXPR{CSTParser.UnarySyntaxOpCall} && sig.args[1].args[1].args[2].args[1] isa EXPR{CSTParser.OPERATOR{CSTParser.DeclarationOp,Tokens.DECLARATION,false}})
        push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.parameterisedDeprecation}((offset + sig.args[1].args[1].fullspan):(offset + sig.args[1].fullspan), [], "Use of deprecated parameter syntax"))
        ...
        push!(last(L.diagnostics).actions, CSTParser.Diagnostics.TextEdit((offset + sig.args[1].args[1].fullspan):(offset + sig.args[1].fullspan), ""))
    end
    ...
    for i = 2:length(sig.args)
        ...
        if !(arg isa EXPR{P} where P <: CSTParser.PUNCTUATION)
            ...
            if arg_id in argnames
                ...
                push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.DuplicateArgumentName}(offset + (0:sig.fullspan-tws), [], "Use of duplicate argument names"))
            end

            push!(argnames, arg_id)
            if fname in argnames
                tws = CSTParser.trailing_ws_length(CSTParser.get_last_token(sig))
                push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.DuplicateArgumentName}(offset + (0:sig.fullspan-tws), [], "Use of function name as argument name"))
            end
...
function lint(x::EXPR{CSTParser.Mutable}, s::TopLevelScope, L::LintState, server, istop)
    if x.args[1] isa EXPR{CSTParser.KEYWORD{Tokens.TYPE}}
        push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.typeDeprecation}(s.current.offset + (0:4), [CSTParser.Diagnostics.TextEdit(s.current.offset + (0:x.args[1].fullspan), "mutable struct ")], "Use of deprecated `type` syntax"))
        ...
        for a in x.args[3].args
            if CSTParser.declares_function(a)
                ...
                if fname.val != name.val && !(fname isa EXPR{CSTParser.InvisBrackets} && fname.args[2] isa EXPR{CSTParser.UnarySyntaxOpCall} && fname.args[2].args[1] isa EXPR{CSTParser.OPERATOR{CSTParser.DeclarationOp,Tokens.DECLARATION,false}})
                    push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.MisnamedConstructor}(offset + (0:a.fullspan), [], "Constructor name does not match type name"))
                end
            end
            offset += a.fullspan
        end
...
function lint(x::EXPR{CSTParser.Struct}, s::TopLevelScope, L::LintState, server, istop)
    if x.args[1] isa EXPR{CSTParser.KEYWORD{Tokens.IMMUTABLE}}
        push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.immutableDeprecation}(s.current.offset + (0:9), [CSTParser.Diagnostics.TextEdit(s.current.offset + (0:x.args[1].fullspan), "struct ")], "Use of deprecated `immutable` syntax"))
    end
    name = CSTParser.get_id(x.args[2])
    nsEx = make_name(s.namespace, name.val)
    if haskey(s.symbols, nsEx) && !(first(first(s.symbols[nsEx])[2]) == s.current.offset)
...
function lint(x::EXPR{CSTParser.Bitstype}, s::TopLevelScope, L::LintState, server, istop)
    ...
    push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.bitstypeDeprecation}(s.current.offset + (0:8), [CSTParser.Diagnostics.TextEdit(s.current.offset + (0:(x.fullspan)), string("primitive type ", Expr(x.args[3])," ", Expr(x.args[2]), " end"))], "This specification for primitive types is deprecated"))
...
function lint(x::EXPR{CSTParser.TypeAlias}, s::TopLevelScope, L::LintState, server, istop)
    ...
    push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.typealiasDeprecation}(s.current.offset + (0:9), [CSTParser.Diagnostics.TextEdit(s.current.offset + (0:(x.fullspan - tws)), string("const ", Expr(x.args[2]), " = ", Expr(x.args[3])))], "This specification for type aliases is deprecated"))
end
...
function _lint_range(x::EXPR{CSTParser.BinaryOpCall}, s::TopLevelScope, L::LintState)
    if ((x.args[2] isa EXPR{CSTParser.OPERATOR{CSTParser.ComparisonOp,Tokens.IN,false}} || x.args[2] isa EXPR{CSTParser.OPERATOR{CSTParser.ComparisonOp,Tokens.ELEMENT_OF,false}}))
        if x.args[3] isa EXPR{L} where L <: LITERAL
            push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.LoopOverSingle}(s.current.offset + (0:x.fullspan), [], "You are trying to loop over a single instance"))
        end
    else
        push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.RangeNonAssignment}(s.current.offset + (0:x.fullspan), [], "You must assign (using =, in or ∈) in a range")) 
    end
end
...
function lint(x::EXPR{CSTParser.If}, s::TopLevelScope, L::LintState, server, istop) 
    ...
    if cond isa EXPR{CSTParser.BinarySyntaxOpCall} && cond.args[2] isa EXPR{OP} where OP <: CSTParser.OPERATOR{CSTParser.AssignmentOp}
        push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.CondAssignment}(s.current.offset + x.args[1].fullspan + (0:cond.fullspan), [], "An assignment rather than comparison operator has been used"))
    end
    if cond isa EXPR{LITERAL{Tokens.TRUE}}
        if length(x.args) == 6
            push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.DeadCode}(s.current.offset + x.args[1].fullspan + cond.fullspan + x.args[3].fullspan + x.args[4].fullspan + (0:x.args[5].fullspan), [], "This code is never reached"))
        end
    elseif cond isa EXPR{LITERAL{Tokens.FALSE}}
        push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.DeadCode}(s.current.offset + x.args[1].fullspan + (0:x.args[2].fullspan + x.args[3].fullspan), [], "This code is never reached"))
    end
    invoke(lint, Tuple{EXPR,TopLevelScope,LintState,LanguageServerInstance,Bool}, x, s, L, server, istop)
end
...
function lint(x::EXPR{CSTParser.While}, s::TopLevelScope, L::LintState, server, istop) 
    ...
    if x.args[2] isa EXPR{CSTParser.BinarySyntaxOpCall} && x.args[2].args[2] isa EXPR{OP} where OP <: CSTParser.OPERATOR{CSTParser.AssignmentOp}
        push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.CondAssignment}(s.current.offset + x.args[1].fullspan + (0:x.args[2].fullspan), [], "An assignment rather than comparison operator has been used"))
    elseif x.args[2] isa EXPR{LITERAL{Tokens.FALSE}}
        push!(L.diagnostics, CSTParser.Diagnostic{CSTParser.Diagnostics.DeadCode}(s.current.offset + (0:x.fullspan), [], "This code is never reached"))
    end
    invoke(lint, Tuple{EXPR,TopLevelScope,LintState,LanguageServerInstance,Bool}, x, s, L, server, istop)
end
