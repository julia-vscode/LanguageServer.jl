...
mutable struct LanguageServerInstance
    ...
    function LanguageServerInstance(@nospecialize(pipe_in), @nospecialize(pipe_out), env_path="", depot_path="", err_handler=nothing, symserver_store_path=nothing, download=true, symbolcache_upstream = nothing, julia_exe_path = nothing)
        new(
            ...
            env_path,
            depot_path,
            SymbolServer.SymbolServerInstance(depot_path, symserver_store_path, julia_exe_path; symbolcache_upstream=symbolcache_upstream),
            Channel(Inf),
