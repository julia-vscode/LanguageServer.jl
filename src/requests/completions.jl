...
using REPL.REPLCompletions: sorted_keywords

@inbounds function kw_completion(doc, s, ppt, pt, t, CIs, offset)
    i = first(searchsorted(sorted_keywords, s))
    n = length(sorted_keywords)
    while i ≤ n && startswith(sorted_keywords[i], s)
        keyword = sorted_keywords[i]
        push!(CIs, CompletionItem(keyword, 13, keyword, TextEdit(Range(doc, offset:offset), keyword[length(s)+1:end])))
        i += 1
    end
end
