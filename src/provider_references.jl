...
function references(doc, offset, server)
    ...
    if y isa IDENTIFIER
        ...
        if haskey(s.symbols, ns_name)
            ...
            toplevel(server.documents[filepath_from_uri(rootfile)].code.ast, s, server)
            s.current.offset = 0
            L = LintState([], [], [])
            R = RefState(ns_name, var_def, [])
            references(server.documents[filepath_from_uri(rootfile)].code.ast, s, L, R, server, true)
            for (loc, uri1) in R.refs
                doc1 = server.documents[filepath_from_uri(uri1)]
...
function references(x::EXPR{Call}, s::TopLevelScope, L::LintState, R::RefState, server, istop)
    if isincludable(x)
        ...
        if haskey(server.documents, filepath_from_uri(file))
            ...
            incl_syms = references(server.documents[filepath_from_uri(file)].code.ast, s, L, R, server, istop)
            s.current = oldpos
        end
    else
