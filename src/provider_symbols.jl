...
function process(r::JSONRPC.Request{Val{Symbol("textDocument/documentSymbol")},DocumentSymbolParams}, server) 
    ...
    syms = map(Parser.get_symbols(doc.blocks.ast)) do v
        rng = Range(Position(get_position_at(doc, first(v[3]))..., one_based=true), Position(get_position_at(doc, last(v[3]))..., one_based=true))
        SymbolInformation(string(v[1]), SymbolKind(v[2]), Location(uri, rng))
    end
